...
import com.jekis.dogglers.BaseTest.DrawableMatcher.withDrawable
...
open class BaseTest {
    ...
    val lastPosition = DataSource().loadData().size - 1
     ...
     */
    private fun hasListItemContent(imageResId: Int, name: String, breed: Int, age: String, hobbies: String) {

        onView(withText(name))
            .check(matches(isDisplayed()))
        onView(withDrawable(imageResId))
            .check(matches(isDisplayed()))
        onView(withText(age))
            .check(matches(isDisplayed()))
        onView(withText(breed))
            .check(matches(isDisplayed()))
    ...
    fun checkFirstPosition() {
        hasListItemContent(R.drawable.dog_1,"Sheree", R.string.dog1_name, "2", "Bring a ball")
    }
